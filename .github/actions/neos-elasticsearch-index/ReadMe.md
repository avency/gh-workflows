# Neos Build ElasticSearch Index

Build the elasticsearch index with the default command or with the queue.

```yaml
name: Neos Build Elasticsearch Index
run-name: Build Elasticsearch Index for ${{ inputs.environment }}
...
jobs:
  init:
    ...
    steps:
      - uses: avency/gh-workflows/.github/actions/neos-elasticsearch-index@main
        with:
